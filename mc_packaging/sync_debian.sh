...
export PACKAGE="lua-resty-core"
export PPA="${PACKAGE}"
export PPA="nginx"
export PPASUFFIX="ppa-${PPA}-"
export REPO="https://salsa.debian.org/lua-team/${PACKAGE}.git"
...
HTTPLUAVER=$(grep -E "libnginx-mod-http-lua.*<<.*" "$W"/debian/control|sed -re "s/.*[.]([0-9]+)\.[0-9]+~.*/\1/g")
sed -i -re "s/<< 1:0.10.22.1~/<< 1:0.10.$(($HTTPLUAVER+1))/g" "$W/"debian/control
fi
#
...
git rco debian/lua5.4.dh-lua.conf
exec 1>&1 2>&2
rm "${logfile}.pipe"
